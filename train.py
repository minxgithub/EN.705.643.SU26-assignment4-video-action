...
def train(dataloaders, model, criterion, optimizer, scheduler,  # pylint: disable=too-many-arguments, too-many-positional-arguments, too-many-locals
          device,optim_model_wts_dir, n_epochs=30, wandb_run=None):
    """
    ...
    for epoch in range(n_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            print(
                f"Best model weights are updated at epoch {epoch+1} "
...
def get_epoch_loss(model, criterion, dataloader, device, optimizer=None):  # pylint: disable=too-many-locals
    """
    ...
    if processed_samples == 0:
        raise RuntimeError(
            "No valid video samples were processed during this epoch."
        )

    loss = running_loss / processed_samples
    accuracy = running_total_correct_preds / processed_samples
